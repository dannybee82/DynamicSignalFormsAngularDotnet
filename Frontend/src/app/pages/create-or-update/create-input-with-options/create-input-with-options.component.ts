...
import { allowedPropertyTypesRadioGroup, PropertyType } from '../../../models/enums/property-type.enum';
...
@Component({
  selector: 'app-create-input-with-options',
  imports: [AllMaterialsModule, FormField, ValidationsComponent, OptionsComponent],
  templateUrl: './create-input-with-options.component.html',
  styleUrl: './create-input-with-options.component.scss',
})
export class CreateInputWithOptionsComponent  implements OnInit, OnDestroy {
  ...
  inputForm = form<SignalFormField>(this.defaultValues, (f) => {
    ...
    validate(f.options, ({ value}) => {
      if(value().length === 0) {
        return { kind: 'options-length-zeor', message: 'There are no options added' };
      }
  ...
  allowedPropertyTypesRadioGroup: PropertyType[] = allowedPropertyTypesRadioGroup;
  ...
  ngOnInit(): void {
    ...
    switch(this.formType()) {
      case ShowFormType.INPUT_TEXT_AUTOCOMPLETE:
        this.inputForm.propertyType().setControlValue(PropertyType.STRING);
        break;
      case ShowFormType.INPUT_SELECT:
        this.inputForm.propertyType().setControlValue(PropertyType.STRING);
        break;
      case ShowFormType.INPUT_SELECT_MULTI:
        this.inputForm.propertyType().setControlValue(PropertyType.STRING);
        break;
      case ShowFormType.RADIO_BUTTON_GROUP:
        this.inputForm.propertyType().setControlValue(PropertyType.UNKNOWN);
        break;
    }
  }
  ...
  getFormType(): typeof ShowFormType {
    return ShowFormType;
  }
